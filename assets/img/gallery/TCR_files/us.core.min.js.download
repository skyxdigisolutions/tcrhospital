...
else{this.usMod(mod,!1).get(0).className+=' '+mod+'_'+value;return this}};$us.toBool=function(value){if(typeof value=='string')return(value=='true'||value=='True'||value=='TRUE'||value=='1');if(typeof value=='boolean')return value;return!!parseInt(value)};$us.getScript=function(url){if(!$us.ajaxLoadJs){return!1}
if($us.loadedScripts===undefined){$us.loadedScripts=[]}
if(jQuery.inArray(url,$us.loadedScripts)!==-1){return}
$us.loadedScripts.push(url);var options={dataType:"script",cache:!0,url:url,async:!1};return jQuery.ajax(options)};$us.detectIE=function(){var ua=window.navigator.userAgent;var msie=ua.indexOf('MSIE ');if(msie>0){return parseInt(ua.substring(msie+5,ua.indexOf('.',msie)),10)}
...
$titleCount.html('0%');$barCount.html('0%');$us.scroll.addWaypoint(this,'15%',function(){var current=0,step=40,stepValue=count/40,interval=setInterval(function(){current+=stepValue;step--;$titleCount.html(current.toFixed(0)+'%');$barCount.html(current.toFixed(0)+'%');if(step<=0){$titleCount.html(count+'%');$barCount.html(count+'%');window.clearInterval(interval)}},20);$container.removeClass('initial')})})});jQuery(function($){$('.w-gallery.link_media .w-gallery-list').each(function(){$us.getScript($us.templateDirectoryUri+'/framework/js/jquery.magnific-popup.js');$(this).magnificPopup({type:'image',delegate:'a.w-gallery-item',gallery:{enabled:!0,navigateByImgClick:!0,preload:[0,1],tPrev:$us.langOptions.magnificPopup.tPrev,tNext:$us.langOptions.magnificPopup.tNext,tCounter:$us.langOptions.magnificPopup.tCounter},removalDelay:300,mainClass:'mfp-fade',fixedContentPos:!1})});$('.w-gallery.layout_masonry').each(function(index,gallery){$us.getScript($us.templateDirectoryUri+'/framework/js/jquery.isotope.js');if($(gallery).hasClass('cols_1'))return;var $container=$($(gallery).find('.w-gallery-list')),isotopeOptions={layoutMode:'masonry',isOriginLeft:!$('body').hasClass('rtl')};if($container.parents('.w-tabs-section-content-h').length){isotopeOptions.transitionDuration=0}
$container.imagesLoaded(function(){$container.isotope(isotopeOptions);$container.isotope()});$us.$canvas.on('contentChange',function(){$container.imagesLoaded(function(){$container.isotope()})})})});(function($){$.fn.wSlider=function(){return this.each(function(){$us.getScript($us.templateDirectoryUri+'/framework/js/jquery.royalslider.min.js');var $this=$(this),$slider=$this.find('.royalSlider'),$options=$this.find('.w-slider-json'),options=$options[0].onclick()||{};$options.remove();if(!$.fn.royalSlider){return}
...
$us.$canvas.on('contentChange',function(){$slider.parent().imagesLoaded(function(){slider.updateSliderSize()})})})};$(function(){jQuery('.w-slider').wSlider()})})(jQuery);!function($){"use strict";$us.WPortfolio=function(container,options){this.init(container,options)};$us.WPortfolio.prototype={init:function(container,options){this.$container=$(container);if(this.$container.usMod('position')!='isotope'){return}
$us.getScript($us.templateDirectoryUri+'/framework/js/jquery.isotope.js');this.$filters=this.$container.find('.g-filters-item');this.$list=this.$container.find('.w-portfolio-list');this.$items=this.$container.find('.w-portfolio-item');this.$pagination=this.$container.find('.g-pagination');this.$loadmore=this.$container.find('.g-loadmore');this.paginationType=this.$pagination.length?'regular':(this.$loadmore.length?'ajax':'none');this.items={};this.curCategory='*';this.loading=!1;this.$items.each(function(index,item){this.items[parseInt(item.getAttribute('data-id'))]=$(item)}.bind(this));this.isotopeOptions={itemSelector:'.w-portfolio-item',layoutMode:'masonry',masonry:{},isOriginLeft:!$('.l-body').hasClass('rtl')};if(this.$container.find('.w-portfolio-item.size_1x1').length){this.itemWidth=1;this.isotopeOptions.masonry.columnWidth='.size_1x1'}else if(this.$container.find('.w-portfolio-item.size_1x2').length){this.itemWidth=1;this.isotopeOptions.masonry.columnWidth='.size_1x2'}else{this.itemWidth=2;this.isotopeOptions.masonry.columnWidth='.w-portfolio-item'}
...
$us.WMapsGeocodesStack[$us.WMapsCurrentGeocode]()}else{$us.WMapsGeocodesRunning=!1}};$us.WMaps=function(container,options){$us.getScript($us.templateDirectoryUri+'/framework/js/gmaps.min.js');this.$container=$(container);var $jsonContainer=this.$container.find('.w-map-json'),jsonOptions=$jsonContainer[0].onclick()||{},$jsonStyleContainer=this.$container.find('.w-map-style-json'),jsonStyleOptions,markerOptions,shouldRunGeoCode=!1;$jsonContainer.remove();if($jsonStyleContainer.length){jsonStyleOptions=$jsonStyleContainer[0].onclick()||{};$jsonStyleContainer.remove()}
